...
void token_lctx(LexContext& lctx, int debug_flags){
    ...
    while (1){
        ...
        if (_c == '\0'){lctx.emit(Token{TK_EOF, lctx.line}); break;};
        ...
        if (punctcode != TK_EMPTY){
            if (dmsg){cout << "\nMatched Punctuation!";}
            lctx.cnext();
            lctx.emit(Token{punctcode, lctx.line});
            continue;
        }
        ...
        if (isalpha(_c) || _c == '_'){
            ...
            if(lexm == "if"){lctx.emit(Token{TK_IF, lctx.line}); continue;}
            if(lexm == "else"){lctx.emit(Token{TK_ELSE, lctx.line}); continue;}
            if(lexm == "func"){lctx.emit(Token{TK_FN, lctx.line}); continue;}
            if(lexm == "get"){lctx.emit(Token{TK_GET, lctx.line}); continue;}
            if(lexm == "return"){lctx.emit(Token{TK_RET, lctx.line}); continue;}
